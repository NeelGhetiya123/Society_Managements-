...
class AnnouncementsController extends Controller
{
    public function showAnnouncementList(Request $request)
    {
        ...
        if (!$adminId || !$LoggedAdminInfo = Admin::find($adminId)) {
            return redirect()->route('admin.login')->with('fail', 'You must be logged in to access the Announcements page');
        }

        $perPage = $request->get('perPage', 10);
        $announcements = Announcement::paginate($perPage);

        return view('admin.announcement', compact('LoggedAdminInfo', 'announcements'));
    }
